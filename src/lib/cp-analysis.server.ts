...
type RawTopic = {
  materi?: unknown;
  kompetensi?: unknown;
  fokusBelajar?: unknown;
  tingkatKesulitan?: unknown;
  pertemuan?: unknown;
  alokasi?: unknown;
  tp?: unknown;
  atpDasar?: unknown;
...
function normalizeTopic(raw: RawTopic, index: number, allocation: string): MasterTopic | undefined {
  ...
  if (!materi || !kompetensi || !fokusBelajar || GENERIC_TITLES.has(normalized(materi)))
    return undefined;

  const no = index + 1;
  const seenTp = new Set<string>();
  const tp = array<RawTp>(raw.tp)
    .map((item, tpIndex) => {
      ...
      if (
        ...
        !indikator ||
        !kktp ||
        !OPERATIONAL_VERBS.some((verb) => rumusan.toLowerCase().includes(verb)) ||
        seenTp.has(key)
      )
        return undefined;
      seenTp.add(key);
      return {
        kode: text(item.kode, `TP.${no}.${tpIndex + 1}`),
        rumusan,
        indikator,
        kktp,
        level: text(item.level, "C3"),
      };
    })
  ...
  const orderedTp = requestedOrder.length
    ? [...tp].sort((a, b) => {
  ...
  return {
    ...
    tingkatKesulitan: text(raw.tingkatKesulitan, "Menengah"),
    tp: orderedTp,
    pemahamanBermakna: fokusBelajar,
    pertanyaanPemantik: [],
    model: text(model.nama, "Inquiry Learning"),
    alasanModel: text(model.alasan),
    sintaks: [],
    metode: text(model.metode),
    lintasDisiplin: "",
    dimensiProfil: [],
    materiFaktual: text(materials.faktual),
    materiKonseptual: text(materials.konseptual),
    materiProsedural: text(materials.prosedural),
    materiMetakognitif: text(materials.metakognitif),
    pengetahuanAwal: "",
    minatBelajar: "",
    kebutuhanBelajar: "",
    kemitraan: "",
    ...
    uraianMateri: [],
    petaKonsep: [],
    rangkuman: fokusBelajar,
    pertemuanRinci: [],
    lkpd: { alatBahan: [], langkah: [], pertanyaan: [] },
    asesmen: {
      diagnostik: [],
      formatif: assessments.map((row) => ({
        aspek: text(row.dimensi),
        indikator: text(row.bukti),
        teknik: text(row.teknik),
        instrumen: text(row.instrumen),
      })),
      sumatif: [],
    },
    kisi: [],
...
function parseAndValidate(rawText: string, data: CpAnalysisInputType): MasterData {
  ...
  const topics = array<RawTopic>(parsed.topics)
    .filter((topic) =>
      [
        ...significantWords(
          `${text(topic.materi)} ${text(topic.kompetensi)} ${text(topic.fokusBelajar)}`,
        ),
      ].some((word) => cpWords.has(word)),
    )
    .filter((topic) => {
  ...
  if (!topics.length)
    throw new AiError(
      "AI_VALIDATION_ERROR",
      "Hasil analisis AI belum dapat ditelusuri ke CP atau belum memiliki TP yang valid. Coba analisis ulang.",
    );
  ...
  return {
    ...data,
    mainCompetencies: strings(parsed.mainCompetencies),
    supportingCompetencies: strings(parsed.supportingCompetencies),
    topics,
  };
}
...
{"mainCompetencies":["kompetensi utama spesifik dari CP"],"supportingCompetencies":["kompetensi pendukung spesifik dari CP"],"topics":[{"materi":"judul topik spesifik dari CP","kompetensi":"kompetensi CP yang dituju","fokusBelajar":"fokus terukur topik","tingkatKesulitan":"Dasar | Menengah | Lanjut","pertemuan":2,"alokasi":"2 x (${data.alokasiPerPertemuan})","tp":[{"kode":"TP.1.1","rumusan":"kata kerja operasional + konten + konteks","indikator":"bukti teramati","kktp":"kriteria keberhasilan spesifik","level":"C3"}],"atpDasar":[{"urutan":1,"kodeTp":"TP.1.1","alasanUrutan":"prasyarat/progresi singkat"}],"materiInti":{"faktual":"...","konseptual":"...","prosedural":"...","metakognitif":"..."},"model":{"nama":"Inquiry Learning","alasan":"kaitan dengan kompetensi","metode":"metode relevan"},"asesmen":[{"tpCodes":["TP.1.1"],"dimensi":"Pengetahuan | Keterampilan | Sikap","teknik":"...","instrumen":"...","bukti":"bukti pencapaian TP"}]}]}`;
  ...
  const cacheKey = buildCacheKey("cp-analysis-v3-core", {
    jenjang: data.jenjang,
    kelas: data.kelas,
    fase: data.fase,
    ...
    ({ text: output } = await askAI({
      ...
      retry: {
        ...
        onRetry: (_error, attempt, delayMs) =>
          console.warn(`[analyzeCP] retry ${attempt} in ${Math.round(delayMs)}ms`),
      },
    }));
  } catch (error) {
  ...
  return {
    topics: master.topics.map(({ no, materi, kompetensi, pertemuan, alokasi }) => ({
      no,
      materi,
      kompetensi,
      pertemuan,
